...
|----------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Format Date** [`Date`] to [`Format String`]              | Converts a date into a readable text format. When formatting a date, use these letters to define how it looks:<ul><li><b>Year</b>: <code>YYYY</code> (2024), <code>YY</code> (24)</li><li><b>Month</b>: <code>MMMM</code> (September), <code>MMM</code> (Sep), <code>MM</code> (09), <code>M</code> (9)</li><li><b>Day</b>: <code>Dddd</code> (Friday), <code>Ddd</code> (Fri), <code>DD</code> (05), <code>D</code> (5)</li><li><b>Time</b>: <code>HH</code> (24hr), <code>hh</code> (12hr), <code>mm</code> (minutes), <code>ss</code> (seconds), <code>a</code> (am/pm)</li></ul> |
...
| **Slice Text** [`Text`] from [`Start`] to [`End`]                    | Cuts out a specific portion of text.<br> **💡Pro-Tip**: Use negative numbers to count backward from the end of the text.                        |
...
2. **Change Case** in `Query Value` to `uppercase`
3. **Combine Text** in `"WELCOME "` and `Cased Text` with `" "`
4. **Change** `#welcome-label`'s `Text` to `Combined Text`
